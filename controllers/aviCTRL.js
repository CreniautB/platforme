        ...
        const promise1 = new Promise((resolve, reject) => {
            ...
            fs.readdir(directoryPath, function (err, files) {
                ...
                for (i = 1; i < (files.length - 1); i++) {
                    ...
                    exec(`ffmpeg -i ${'public/videoSource/'+req.params.id+'/'+`${i}`+'.webm'} -i ${directoryPath+'/webCam'+i+'.webm'} -filter_complex "[0:v]scale=10:5,setsar=1[l];[1:v]scale=10:5,setsar=1[r];[l][r]hstack;[0][1]amix" -async 0  ${directoryPathVideo+'/webCam'+i+'.webm'} `, (error, stdout, stderr) => {
                        if (error) {
